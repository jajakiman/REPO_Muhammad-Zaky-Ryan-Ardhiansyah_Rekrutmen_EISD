...
<div class="field">
    ...
    <input id="name" name="name" value="{{ old('name', $campus->name ?? '') }}" required aria-describedby="name-error">
    @error('name')<p class="field-error" id="name-error">{{ $message }}</p>@enderror
</div>
<div class="field">
    <label for="address">Alamat</label>
    <textarea id="address" name="address" rows="3" aria-describedby="address-error">{{ old('address', $campus->address ?? '') }}</textarea>
    @error('address')<p class="field-error" id="address-error">{{ $message }}</p>@enderror
</div>
<div class="field-grid">
    <div class="field">
        <label for="latitude">Latitude</label>
        <input id="latitude" name="latitude" type="number" step="0.0000001" min="-90" max="90" value="{{ old('latitude', $campus->latitude ?? '') }}" aria-describedby="latitude-error">
        @error('latitude')<p class="field-error" id="latitude-error">{{ $message }}</p>@enderror
    </div>
    <div class="field">
        <label for="longitude">Longitude</label>
        <input id="longitude" name="longitude" type="number" step="0.0000001" min="-180" max="180" value="{{ old('longitude', $campus->longitude ?? '') }}" aria-describedby="longitude-error">
...
<div class="field">
    ...
    <select id="is_active" name="is_active" required aria-describedby="is_active-error">
