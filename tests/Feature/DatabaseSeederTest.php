...
class DatabaseSeederTest extends TestCase
{
    ...
    public function test_it_seeds_demo_accounts_areas_locations_and_facilities_idempotently(): void
    {
        ...
        $this->assertDatabaseCount('campus_areas', 2);
        $this->assertDatabaseCount('campus_locations', 4);
        $this->assertDatabaseCount('location_accessibility_features', 11);
        $this->assertDatabaseHas('campus_locations', [
            'name' => 'Gedung Tokong Nanas (GKB)',
            'location_type' => 'building',
            'accessibility_status' => 'accessible',
        ]);
        $this->assertDatabaseHas('campus_locations', ['name' => 'Masjid Syamsul Ulum Telkom']);
        $this->assertDatabaseMissing('campus_locations', ['name' => 'Gedung Isola (Rektorat UPI)']);
        $this->assertDatabaseMissing('campus_locations', ['name' => 'Gedung Magnesit (Rektorat)']);
